...
public class Day10 {
	...
	public static void part1() throws URISyntaxException, IOException {
		...
		var particles = lines.stream()
				.map(l -> new Particle(new Coord(l.split("<")[1].split(",")[0], l.split(",")[1].split(">")[0]),
		...
		while (atLeastOneSingle) {
			...
			atLeastOneSingle = particles.parallelStream()
					.filter(p1 -> particles.parallelStream().filter(p2 -> !p1.equals(p2)
							&& (Math.abs(p1.position.x - p2.position.x) + Math.abs(p1.position.y - p2.position.y)) <= 2)
							.count() > 0)
		...
		LongStream.rangeClosed(minY, maxY).forEach(j -> {
			LongStream.rangeClosed(minX, maxX).forEach(i -> {
				System.out.print(particles.stream().filter(p -> p.position.x == i && p.position.y == j).findAny()
						.orElse(null) != null ? "#" : ".");
			});
			System.out.println();
		});
